    ...
    */
    ...
    if (old_vpn != NULL)
    {
        ...
        // {
            ...
            // uart_printf("bank : %d, vblock : %d, vcount : %d", bank, vblock, get_vcount(bank,vblock) -1);
    ...
    // g_misc_meta[bank].written_time[page_num] = cur_cnt;
    ...
    write_dram_32(OOB_WRITTEN_TIME_ADDR + ((bank * PAGES_PER_BLK + page_num) * sizeof(UINT32)), cur_cnt);
    ...
    //     uart_printf("[WRITE!!!] bank : %d, cur_vpn : %d, lpn : %d, old_vpn : %d, written_time : %d, pid : %d, fid : %d, offset : %d", new_vpn, lpn, old_vpn, cur_cnt, f.pid, f.fid, f.offset);
    
    // uart_printf("");
    // uart_print("*********************************************");
    // uart_printf("DATA : lpn : %d, old_vpn : %d, WT : %d, pid : %d, fid : %d, offset : %d, vpn : %d", lpn, old_vpn, cur_cnt, f.pid, f.fid, f.offset, new_vpn);
    // UINT32 a, b, c, d, e;
...
static void move_to_backup_zone_jw(UINT32 const bank, UINT32 const vpn, UINT32 const _lpn, UINT32 const _old_vpn, UINT32 const _written_time, UINT32 const _pid, UINT32 const _fid, UINT32 const _offset, UINT32 const prev_WT)
{
    ...
    {
        // uart_printf("Invalidation Time : %d, PVSSD_POLICY.ret_time : %d, cur_cnt : %d", prev_WT, pvssd_policy[_pid].ret_time, cur_cnt);
        // uart_printf("ET %d vs %d CURRENT TIME ", prev_WT + pvssd_policy[_pid].ret_time, cur_cnt);
        
        if (prev_WT + pvssd_policy[_pid].ret_time < cur_cnt) {
    ...
    do{
        ...
        // if(cur.old_vpn != NULL) 
        //     uart_printf("PREV!!! -> bank : %d, vpn : %d, lpn : %d, old_vpn : %d, WT : %d, pid : %d, fid : %d, offset : %df", prev.bank, prev.vpn, prev.lpn, prev.old_vpn, prev.written_time, prev.pid, prev.fid, prev.offset); 

        // 현재 버전을 백업존으로 복사한다.
        // get an index of the bucket
        UINT8 idx = get_bucket(cur.expiration_time); // Need to have hash function
...
//////////////////////////////////////////////////////////////////////
///jw
// static void set_backup_blk_addr(UINT32 index, UINT32 write_ppn)
// {
//     UINT32 cnt = cur_backup_write_cnt[index];
//     cur_backup_write_cnt[index]++;
//     write_dram_32(BACKUP_BLK_ADDR + (index * sizeof(UINT32) * BLOCK_PER_BUCKET + cnt), write_ppn);
// }

//아래는 백업존으로부터 만료된 블록을 회수하기 위한 함수3개임
//cnt는 0부터 cur_backup_write_cnt[index]까지 호출하여 reclaim하면 됨.
// static UINT32 get_backup_blk_addr(UINT32 index, cnt)
// {
//     return read_dram_32(BACKUP_BLK_ADDR + (index * sizeof(UINT32) * BLOCK_PER_BUCKET + cnt));
// }

// static UINT32 get_backup_blk_cnt(UINT32 index)
// {
//     return cur_backup_write_cnt[index];
// }

// static void reclaim_backup_blk_addr(UINT32 index)
// {
//     cur_backup_write_cnt[index] = 0;
// }
//////////////////////////////////////////////////////////////////////
...
static UINT32 assign_new_write_vpn(UINT32 const bank)
{
    ...
    if(error_correction < NUM_BANKS) {
        uart_printf("%dth assign for bank %d", error_correction++, bank);
    ...
    if ((write_vpn % PAGES_PER_BLK) == (PAGES_PER_BLK - 2))
    {
        ...
        //         uart_printf("%dth page address : 0x%x", k, OOB_WRITTEN_TIME_ADDR + ((bank * PAGES_PER_BLK + k) * sizeof(UINT32)));
        ...
        //         uart_printf("[FLUSH!!!] write_vpn : %d, bank : %d, vblock : %d, page_num : %d, lpn : %d, old_vpn : %d, prev_WT : %d, pid : %d, fid : %d", vblock*PAGES_PER_BLK+k, bank, vblock, k, a, b, c, d, e);
        //     }
        // }   
        ...
        //         uart_printf("bank : %d, vt_vblock : %d, lpn[%d] : %d", bank, vblock, k, g_misc_meta[bank].lpn[k]);
...
static UINT32 assign_new_backup_write_ppn(UINT32 const hash_index)
{
    ...
    UINT32 ppn = pblock + page_no;
    
    
    if (delta_offset[hash_index] != 0 && delta_offset[hash_index] % 5 == 0)
...
static void modified_garbage_collection(UINT32 const bank)
{
    ...
    // insert_free_blk_pool(bank, vt_vblock, f_pool);
...
// UINT32 ftl_recovery (UINT32 const lba, UINT32 const fid, UINT32 const r_time)
// {
...
//     cur_vpn = get_vpn(lpn);
//     cur_WT = g_misc_meta[bank].written_time[cur_vpn % PAGES_PER_BLK];
