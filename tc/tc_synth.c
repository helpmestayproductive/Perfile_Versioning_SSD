...
#define TOTAL_SIZE (6*1024*1024) // 25GB
...
void ftl_test(void)
{
    ...
    pvssd_policy[3].ret_time = 10000000;

    uart_printf("Initial write of %dGB start!", TOTAL_SIZE / (1024*1024));
    while (size != TOTAL_SIZE)
    {
        cur_cnt++;
        // uart_printf ("lba 0x%x, size %d, time %d", lba, size, cur_cnt);
        ftl_write (lba, SECTORS_PER_FILE, n);
        lba += SECTORS_PER_FILE; size += FILE_SIZE;
        if (size % (1024 * 1024) == 0)
            uart_printf("%dGB has been written!", size/(1024*1024));
    }

    uart_printf("Initial overwrite of %dGB start!", TOTAL_SIZE / (1024*1024));
    size = 0; lba = 0;
    while (size != TOTAL_SIZE)
    {
        cur_cnt++;
    ...
    uart_printf("2nd Overwrite of %dGB start; %dGB so far", TOTAL_SIZE / (1024*1024), TOTAL_SIZE*2 / (1024*1024));
    size = 0; lba = 0;
    while (size != TOTAL_SIZE)
    {
        cur_cnt++;
    ...
    uart_printf("3rd Overwrite of %dGB start", TOTAL_SIZE / (1024*1024));
    size = 0; lba = 0;
    while (size != TOTAL_SIZE)
    {
        cur_cnt++;
    ...
    uart_print("**********Experiments Done!**********");

    UINT32 i;
    for (i=0; i<NUM_BANKS; i++)
        uart_printf("The number of written pages bank[%d] : %d", i, written_page_num[i]);

    uart_print("************************************");

    for (i=0; i<NUM_BANKS; i++)
        uart_printf("The number of backed-up pages bank[%d] : %d", i, backuped_page_num[i]);
